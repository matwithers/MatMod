...
exports.ModifyLoot = void 0;
const GlobalValues_1 = require("./GlobalValues");
const ModifyLoot = (container) => {
    GlobalValues_1.globalValues.config.debug && GlobalValues_1.globalValues.Logger.info(" ==> ModifyLoot Started");
    const globals = GlobalValues_1.globalValues.database.globals;
    const loot = GlobalValues_1.globalValues.database.loot;
    // Set Global loot chance modifier (loose loot) Default 0.2
    if (GlobalValues_1.globalValues.config.Loot.modifyGlobalLootChance) {
        globals.GlobalLootChanceModifier = GlobalValues_1.globalValues.config.Loot.globalLootChanceModifier;
    }
    ...
    const ragfairConfig = GlobalValues_1.globalValues.configServer.getConfig("aki-ragfair");
    ragfairConfig.dynamic.blacklist.enableBsgList = false;
    const itemConfig = GlobalValues_1.globalValues.configServer.getConfig("aki-item");
    itemConfig.blacklist = [];
    GlobalValues_1.globalValues.config.debug && GlobalValues_1.globalValues.Logger.info(" ==> ModifyLoot Complete");
};
exports.ModifyLoot = ModifyLoot;
